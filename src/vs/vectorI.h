...
{
  const double denom = std::max(mag(v1) * mag(v2), vs::DTraits<T>::eps());
  const double val = (v1 & v2) / denom;
  return std::acos(std::max(-1.0, std::min(1.0, val)));
}
