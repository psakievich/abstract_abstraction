...
void test_vector_create_impl() {
  abstract::Scalar ds("test", 1);
  auto *ddata = ds.device_data();
  abstract::parallel_for(
      1, DEVICE_LAMBDA(int) {
  ...
  ds.copy_device_to_host();

  EXPECT_NEAR(ds.host_value(), 0.0, tol);
}
...
#define CHECK_ON_GPU(expr1, expr2)                                             \
  {                                                                            \
    abstract::Scalar ds("test", 1.0e16);                                       \
    auto *dv = ds.device_data();                                               \
                                                                               \
    abstract::parallel_for(                                                    \
        1, DEVICE_LAMBDA(int) {                                                \
          auto v1 = expr1;                                                     \
...
void test_device_lists_impl() {
  abstract::Vector<vs::Vector> dvec("vec_test", 3);
  auto *dv = dvec.device_data();
  ...
  dvec.copy_device_to_host();

  std::vector<vs::Vector> htrue{vs::Vector::ihat(), vs::Vector::jhat(),
                                vs::Vector::khat()};

  for (int i = 0; i < 3; ++i) {
    EXPECT_NEAR(vs::mag(htrue[i] - dvec.host_value(i)), 0.0, tol);
  }
}
